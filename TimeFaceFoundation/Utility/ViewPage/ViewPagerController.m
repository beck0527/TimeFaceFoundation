...
#define kIndicatorColor [UIColor colorWithRed:255.0/255.0 green:203.0/255.0 blue:57.0/255.0 alpha:0.75]
#define kTabsViewBackgroundColor [UIColor colorWithRed:234.0/255.0 green:234.0/255.0 blue:234.0/255.0 alpha:1]
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        self.backgroundColor = RGBCOLOR(232, 233, 232);
    }
    return self;
}
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.selected) {
        bezierPath = [UIBezierPath bezierPath];
        // Draw the indicator
        [bezierPath moveToPoint:CGPointMake(_subViewFrame.origin.x, CGRectGetHeight(rect) - 1.0)];
        [bezierPath addLineToPoint:CGPointMake(CGRectGetWidth(rect) - _subViewFrame.origin.x, CGRectGetHeight(rect) - 1.0)];
...
@interface ViewPagerController () <UIGestureRecognizerDelegate,UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIScrollViewDelegate>
...
@synthesize hiddenTab = _hiddenTab;
...
- (void)loadView {
    if (nil != self.nibName) {
        ...
        if ([self.delegate respondsToSelector:@selector(frameForViewPager)]) {
            frame = [self.delegate frameForViewPager];
        }
        else {
            frame = TTScreenBounds();
        }
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    self.screenWidth = TTScreenBounds().size.width;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    _previousOffsetX = scrollView.contentOffset.x;
}
